...
export function HowItWorks() {
  return (
    <section id="how-it-works" className="site-wrap section-pad">
      <Reveal>
      ...
      <Stagger className="border-b border-border" stagger={0.12} role="list">
        {steps.map((s, i) => (
          <StaggerItem key={s.title} role="listitem">
            <div className="grid gap-3 border-b border-border py-6 last:border-b-0 sm:grid-cols-12 sm:items-baseline sm:gap-8">
              <StepNumber n={i + 1} />
              <h3 className="text-xl font-bold tracking-tight text-fg sm:col-span-4">
                {s.title}
              </h3>
      ...
      <Reveal delay={0.1}>
        <p className="mt-12 max-w-2xl text-sm leading-relaxed text-fg-muted">
